...
test("It should return a list of dependencies for a valid package", async () => {
    ...
    const response = await request(defaultServer.app).get(`/api/dependencies/${packageName}/latest`);
    expect(response.status).toBe(200);
    expect(response.body.dependencies).toContainEqual(Fixtures.expressDependencyExample)
    expect(response.body.devDependencies).toContainEqual(Fixtures.expressDevDependencyExample)
});
